        ...
        if (atom in residueDict[abbr]['atoms']):
            ...
            for bondedAtom in bondInfo[atom]:
                if (bondedAtom in residueDict[abbr]['atoms']):
                    ...
                    print
                    "Error: bonded atom=%s not in residue=%s" % (atom, abbr)
        else:
            print
            "Error: bonded atom=%s nt in residue=%s" % (atom, abbr)
    ...
    for residue in root.findall('Residue'):
        ...
        if (isWater):
            ...
            if (residueDict[abbr]['tinkerLookupName'].find('(') > -1):
                ...
                print
                "tinkerLookupName %s %s" % (abbr, residueDict[cResidueName]['tinkerLookupName'])
            else:
    ...
    for resname, res in residueDict.items():
        for atom in res['atoms']:
            ...
            if lookupName in bioTypes:
                ...
                print
                "For %s lookupName=%s not in biotype" % (atom, lookupName)
                if ('parent' in res):
                    ...
                    if (lookupName in bioTypes):
                        ...
                        print
                        "Missing lookupName=%s from biotype" % (lookupName)
    return 0
...
allLines = []
for line in open('amoebabio18_fixed_anglep.prm.txt'):#sys.argv[1]):
    try:
...
while lineIndex < len(allLines):
    ...
    if fields[0] == 'atom':
        ...
        print
        "Field %s not recognized: line=<%s>" % (fields[0], allLines[lineIndex])
        lineIndex += 1
...
print
"Opened %s." % (tinkerXmlFileName)

gkXmlFileName = scalars['forcefield']
gkXmlFileName += '_gk.xml'
gkXmlFile = open(gkXmlFileName, 'w')
print
"Opened %s." % (gkXmlFileName)

today = datetime.date.today().isoformat()
sourceFile = os.path.basename('amoebabio18_fixed_anglep.prm.txt') #sys.argv[1])sys.argv[1])
header = """ <Info>
  <Source>%s</Source>
...
for resname, res in sorted(residueDict.items()):
    if res['include']:
        ...
        for atom in sorted(res['atoms'].keys()):
            ...
            if (typeI < 0):
                print
                "Error: type=%s for atom=%s of residue=%s" % (type, atom, resname)
...
if (isAmoeba):
    ...
    for (index, torsionTorsion) in enumerate(torsionTorsions):
        ...
        for (gridIndex, gridEntry) in enumerate(grid):
            print
            "Gxx %d  %s" % (gridIndex, str(gridEntry))
            if (len(gridEntry) > 5):
    ...
    for multipoleInfo in multipoleArray:
        ...
        outputString += """c0="%s" d1="%s" d2="%s" d3="%s" q11="%s" q21="%s" q22="%s" q31="%s" q32="%s" q33="%s"  """ % (multipoles[0],
                                                                                                                         str(dipoleConversion * float(multipoles[1])), str(dipoleConversion * float(multipoles[2])),
                                                                                                                         str(dipoleConversion * float(multipoles[3])),
                                                                                                                         str(quadrupoleConversion * float(multipoles[4])), str(quadrupoleConversion * float(multipoles[5])),
                                                                                                                         str(quadrupoleConversion * float(multipoles[6])),
                                                                                                                         str(quadrupoleConversion * float(multipoles[7])), str(quadrupoleConversion * float(multipoles[8])),
                                                                                                                         str(quadrupoleConversion * float(multipoles[9])))
        outputString += "/>"
        tinkerXmlFile.write("%s\n" % (outputString))
    ...
    for type in sorted(atomTypes):
        print
        "atom type=%s  %s" % (str(type), str(atomTypes[type]))

    for type in sorted(bioTypes):
        print
        "bio type=%s  %s" % (str(type), str(bioTypes[type]))
    ...
    for multipoleInfo in multipoleArray:
        ...
        if (type in atomTypes):
            ...
            if (element == 'H'):
                ...
                print
                "Warning no overlap scale factor for type=%d element=%s" % (type, element)
        else:
            print
            "Warning no overlap scale factor for type=%d " % (type)
