...
if __name__ == "__main__":
    ...
    parser.add_argument("input_pdb_file", help="Input PDB file", metavar="input_pdb_file")
    parser.add_argument("anchor_residue", help="A residue in the format ChainID.ResidueName.ResidueNumber "
        "which indicates the point to start to build the fake-membrane", metavar="anchor_residue")
    parser.add_argument("--angular_resolution", "-angular_resolution", "-ar", help="Angular resolution, in degrees",
                            dest="angular_resolution", type=float, default=15.)
    parser.add_argument("--radius_resolution", "-radius_resolution", "-rr", help="Radius resolution, in Angstroms",
                            dest="radius_resolution", type=float, default=4.)
    parser.add_argument("--radius_offset", "-radius_offset", "-ro", help="Radius offset, in Angstroms",
                            dest="radius_offset", type=float, default=30.)
    parser.add_argument("--output_pdb_file", "-output_pdb_file", "-o", help="Output PDB file name",
                            dest="output_pdb_file", default="membrane.pdb")
    ...
    ca_residue = molecule.select(f'chain {chain} and resname {res_name} and resnum {res_num} name CA')
    ...
    create_pdb_from_points(args.output_pdb_file, points, atom_type='BJ', res_type='MMB', element='P')
