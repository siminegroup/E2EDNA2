...
# from nupack import *  # unused
from seqfold import dg, fold


def getSeqfoldStructure(sequence, temperature):
    """
    Output the secondary structuer for a given sequence at a given condition
...
def ssToList(ssString):
    ...
    for i in range(len(ssString)):
        if ssString[i] == '(':  # if it's paired
            counter = 0
            for j in range(1, len(ssString[i:])):  # look for the thing that is paired to it
                if ssString[i + j] == '(':
                    counter += 1
                if ssString[i + j] == ')':
                    if counter > 0:
                        counter -= 1
                    elif counter == 0:  # found a paired one
                        if (not i in paired) and (not i+j in paired):  # check for duplicate
                            paired.append(i)
                            paired.append(i + j)
                            pairList.append([i + 1, i + j + 1])  # make pair list in 1-to-n basis
                            break
...
def configToString(config):
    """
    Convert 2d structure 'config' to a dot-bracket string
    :param config:
    :return:
    """
    string = '.' * len(config)  # start every base as unpaired. string: '........'
    for i in range(len(config)):
        if (config[i] == 0) or (string[i] != '.'):  # if the base is unpaired or previously noted, leave it be
            pass
        else:  # create a pair
...
def do2DAgglomerativeClustering(configs, probs, distances, distThreshold=0.1, probThreshold=0.05, minClusters=5):
    ...
    while not converged:
        ...
        for i in range(len(np.unique(labels))):
            ...
            probSums[i] = np.sum(clusterProbs[-1])  # un-normalized
        ...
        if (nThresholdClusters >= minClusters) or (nClusters <= minClusters):  # if we have the desired number of clusters above the threshold, stop
            converged = True
        else:
            distThreshold *= 1.1  # boost the threshold

    # find a representative from each other - product of a priori probability and distance from ensemble members
    clusterReps = []
    ...
    sortedInds = sortedInds[::-1]  # reverse the array
    topReps = [clusterReps[j].astype(int) for j in sortedInds]
...
def getMoleculeSize(structure):
    ...
    :param structure:
    :return: the dimensons of the size
    """
...
def getNucDATraj(u):
    ...
    seg = "A"
    a, b, g, d, e, z, c = [[], [], [], [], [], [], []]  # alpha, beta, gamma, delta, epsilon, zeta
    ...
    for i in range(len(atomList)):  # for each type of dihedral, compute the trajectory for all bases
        my_analysis = Dihedral(atomList[i])
        my_analysis.run()
        dihedrals[:, :, i] = my_analysis.angles
...
def checkMidTrajectoryBinding(structure, trajectory, peptide, sequence, params, cutoffTime=1):
    ...
    try:
        ...
        if detachedTime > cutoffTime:  # if they have been unbound longer than the cutoff
            return True
        else:  # oterhwise keep going
            return False

    except IndexError:
        return False  # if they never got attached, give up
...
class atomDistances(AnalysisBase):  # inherit the superclass AnalysisBase from MDAnalysis
    """
...
def findPairingErrors(config, tt):
    ...
    identify whether any elements are paired with more than one other element
    :param config:
    :param tt:
    ...
    for i in range(len(config)):
        if config[i] != 0:  # if it's paired
            ...
            if config[pairMate] != i + 1:  # indexing - is that thing bonded to 'i'?
                pairingErrors.append([tt, pairMate])
...
def analyzeSecondaryStructure(pairTraj):
    ...
    Identify the equilibrium structure and maybe metastable states
    :param pairList:
    :return:
    """
...
def bindingAnalysis(bindu, freeu, peptide, sequence):
    ...
    if np.nonzero(nContacts[:, 0])[0] != []:
        ...
        closeContactRatio = np.average(nContacts[firstContact:, 0] > 0)  # amount of time that peptide spends in close contact with the aptamer
...
def getConformationChange(bindu, freeu):
    ...
    reducedDifference = np.zeros(n_components)
    for i in range(n_components):  # rather than doing high dimensional probability analysis, we'll instead do differences in averages over dimensions
        ...
        reducedDifference[i] = np.average(bindReducedTrajectory[:, i]) / np.average(np.abs(freeReducedTrajectory[:, i]))
        # the average of all the free trajectories is zero by construction - any non-zero average in the binding trajectory is therefore an aberration.

    reducedDifference = np.average(np.abs(reducedDifference))
...
def getBaseBaseDistTraj(u):
    """
    Given a MDA Universe object containing ssDNA (segment 1)
